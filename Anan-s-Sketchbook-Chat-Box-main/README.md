...
默认已开启自动区域检测：
- 文本模式（`AUTO_DETECT_TEXT_REGION=True`）：每次生成文本图片前，会先在当前底图中寻找最大的亮色矩形空白区域，并将文字自动嵌入该区域。
- 图片模式（`AUTO_DETECT_IMAGE_REGION=True`）：每次贴入图片前，也会执行同样的空白区检测，避免更换底图后图片仍粘贴到旧坐标。

找不到合适空白区时会自动回退到 `TEXT_BOX_TOPLEFT` 和 `IMAGE_BOX_BOTTOMRIGHT` 指定的固定区域。可通过 `TEXT_REGION_BRIGHT_THRESHOLD`、`TEXT_REGION_MIN_WIDTH`、`TEXT_REGION_MIN_HEIGHT`、`TEXT_REGION_PADDING`、`TEXT_REGION_SEARCH_MARGIN` 调整检测效果。

另外，图片贴入默认使用更小的 `IMAGE_PASTE_PADDING`（默认为 2）以尽量填满空白区；当同时输入文字和图片时，会先贴图再叠加文字，确保两者都显示在最终图片中（文字默认贴底显示，可通过 `MIXED_TEXT_MAX_FONT_HEIGHT` 调整字号上限）。
